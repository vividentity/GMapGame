...
var appid1 = '26WTvi3V34E2DgU20paS4sfmROlawrpYQ31JRFIbSaFUxiMAQlJjxFbjZYApxx5veR59'; 
var yuri0 = 'http://api.maps.yahoo.com/ajaxymap?appid=' + appid1;
...
function checkTimer() {
	...
	closeYahooTag();
	if (!gCalled && !yCalled) {
		alert('Your request to the geocoder timed out. Be sure you are connected to the Internet, that you have typed the place name correctly Then try again.');
		return false;
	}
	if (yFound) {
		displayYahooResults();
...
function launchGeocode() {
	query = trim(document.getElementById('search').value); //The base address
